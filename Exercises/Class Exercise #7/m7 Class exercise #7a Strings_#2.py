#The get_login_name function accepts a first name, last name,
#and ID number as arguments. It returns a system login name.
...
#The valid_password function accepts a password as an argument
#and returns either true or false to indicate whether the 
#password is valid. A valid password must be at least 7 
#characters in length, have at least one uppercase letter,
...
def valid_password(password):
    ...
    #begin the validation. Start by testing the password's
    #length.
    if len(password) >= 7:
        correct_length = True

        #test each character and set the appropriate flag
        #when a required character is found.
        for ch in password:
            if ch.isupper():
                has_uppercase = True
            if ch.islower():
        ...
        #If they are, set is_valid to true. Otherwise, set
        #is_valid to false.
